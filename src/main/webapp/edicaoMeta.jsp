<%-- src/main/webapp/edicaoMeta.jsp --%>
...
<%
    ...
    Meta meta = (Meta) request.getAttribute("meta");
    if (meta == null) {
...
<div class="container">
    <div class="form-card">
        ...
        <form action="<%= request.getContextPath() %>/metas" method="post">
            ...
            <input type="hidden" name="idMeta" value="<%= meta.getIdMeta() %>">

            <div class="form-group">
                <label for="descricao">Descrição da Meta:</label>
                <input type="text" id="descricao" name="descricao" value="<%= meta.getDescricao() %>" required>
            </div>
            <div class="form-group">
                <label for="valorAlvo">Valor Alvo:</label>
                <input type="number" id="valorAlvo" name="valorAlvo" step="0.01" min="0.01" value="<%= String.format("%.2f", meta.getValorAlvo()).replace(",", ".") %>" required>
            </div>
            <div class="form-group">
                <label for="valorAtual">Valor Atual Guardado:</label>
                <input type="number" id="valorAtual" name="valorAtual" step="0.01" min="0.00" value="<%= String.format("%.2f", meta.getValorAtual()).replace(",", ".") %>" required>
            </div>
            <div class="form-group">
                <label for="dataAlvo">Data Alvo:</label>
                <input type="date" id="dataAlvo" name="dataAlvo" value="<%= meta.getDataAlvo() %>" required>
            </div>
            <div class="form-group">
                <label for="statusMeta">Status:</label>
                <select id="statusMeta" name="statusMeta" required>
                    <option value="Em Progresso" <%= "Em Progresso".equals(meta.getStatusMeta()) ? "selected" : "" %>>Em Progresso</option>
                    <option value="Pendente" <%= "Pendente".equals(meta.getStatusMeta()) ? "selected" : "" %>>Pendente</option>
                    <option value="Concluída" <%= "Concluída".equals(meta.getStatusMeta()) ? "selected" : "" %>>Concluída</option>
                    <option value="Cancelada" <%= "Cancelada".equals(meta.getStatusMeta()) ? "selected" : "" %>>Cancelada</option>
                </select>
            </div>
            <div class="form-group">
                <label for="prioridade">Prioridade:</label>
                <select id="prioridade" name="prioridade" required>
                    <option value="Baixa" <%= "Baixa".equals(meta.getPrioridade()) ? "selected" : "" %>>Baixa</option>
                    <option value="Média" <%= "Média".equals(meta.getPrioridade()) ? "selected" : "" %>>Média</option>
                    <option value="Alta" <%= "Alta".equals(meta.getPrioridade()) ? "selected" : "" %>>Alta</option>
                    <option value="Urgente" <%= "Urgente".equals(meta.getPrioridade()) ? "selected" : "" %>>Urgente</option>
                </select>
            </div>
            <div class="form-group">
