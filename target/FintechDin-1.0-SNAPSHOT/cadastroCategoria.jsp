<%-- src/main/webapp/cadastroCategoria.jsp --%>
...
<head>
  ...
  <title>Din+ - Cadastrar Categoria</title>
  ...
  <style>
    ...
    .container { flex-grow: 1; display: flex; justify-content: center; align-items: center; padding: 40px; }
    .form-card { background-color: #fff; padding: 40px; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1); width: 100%; max-width: 500px; text-align: center; }
    .form-card h2 { margin-bottom: 30px; color: #333; font-size: 28px; }
    .form-group { margin-bottom: 20px; text-align: left; }
    .form-group label { display: block; margin-bottom: 8px; color: #555; font-weight: bold; }
    .form-group input[type="text"], .form-group select { width: calc(100% - 20px); padding: 12px; border: 1px solid #ddd; border-radius: 5px; font-size: 16px; box-sizing: border-box; }
    .form-group input[type="submit"] { background-color: #f7931e; color: white; padding: 15px 30px; border: none; border-radius: 5px; font-size: 18px; font-weight: bold; cursor: pointer; transition: background-color 0.3s ease; width: 100%; }
    .form-group input[type="submit"]:hover { background-color: #e08010; }
    .error-message { color: #dc3545; margin-top: 15px; font-weight: bold; }
    .success-message { color: #28a745; margin-top: 15px; font-weight: bold; }
  ...
  <div class="form-card">
    ...
    <form action="<%= request.getContextPath() %>/categorias" method="post">
      <div class="form-group">
        ...
        <input type="text" id="nome" name="nome" required placeholder="Ex: Assinaturas, Salário Extra">
      </div>
      <div class="form-group">
